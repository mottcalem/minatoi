        ...
        <CarouselContent className="-ml-4 pb-1">
          {slides.map((category, index) => (
            <CarouselItem
              ...
              aria-label={`${(index % categories.length) + 1} / ${categories.length}`}
            >
              {category.comingSoon ? (
                <div className="relative flex h-64 flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-stone-200 bg-stone-50 px-6 text-center">
                  ...
                  <h3 className="mt-3 font-display text-xl font-bold text-stone-400">
                    {category.label}
                  </h3>
                ...
                <Link
                  ...
                  className="group relative block aspect-square overflow-hidden rounded-2xl border border-stone-100 bg-white shadow-sm transition-shadow hover:shadow-elegant focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-primary"
                >
                  <img
                    src={category.image}
                    alt={category.label}
                  ...
                  <div className="absolute bottom-0 p-5">
                    ...
                    <span className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-white/90 transition-all group-hover:gap-2">
                      Keşfet <span>→</span>
                    </span>
                  </div>
                </Link>
